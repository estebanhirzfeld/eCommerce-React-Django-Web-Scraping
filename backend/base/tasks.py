...
from base.models import Product, ProductImage, Review, Order, OrderItem, Size, Color, ProductAttribute, ProductView, CartItem,WishlistItem
...
# Using Celey to Scrape Products 
# Using Celery to Scrape Products
#
import time
...
def scrape_discover():
    ...
    for store in stores:
        ...
        if len(products) <= 0:
            ...
            existing_links = set(Product.objects.values_list('original_url', flat=True))
...
#     Priority 1: update products with no images and are active
    products_prior_1 = Product.objects.filter(is_active=True, images__isnull=True).order_by('-updatedAt')[:100]
...
#     Priority 2: update products with more views in the last 7 days (model has the last_week function for that) and the updatedAt is older than 7 days, also the product must be active
    products_prior_2 = Product.objects.filter(is_active=True, updatedAt__lt=timezone.now() - timedelta(days=7)).order_by('-last_week')[:100]
...
#     CartItem has Product as a foreign key, so you can use the product__last_week function
    products_prior_3 = CartItem.objects.filter(product__is_active=True, product__updatedAt__lt=timezone.now() - timedelta(days=7)).order_by('-product__last_week')[:100]
...
#     WishlistItem has Product as a foreign key, so you can use the product__last_week function
    products_prior_4 = WishlistItem.objects.filter(product__is_active=True, product__updatedAt__lt=timezone.now() - timedelta(days=7)).order_by('-product__last_week')[:100]
...
#     Priority 5: update products with updatedAt older than 7 days and are active
    products_prior_5 = Product.objects.filter(is_active=True, updatedAt__lt=timezone.now() - timedelta(days=7))[:100]

#     Priority 6: if the 4 priorities don't exceed 8:00 AM keep updating products till 8:00 AM 
    products_prior_6 = Product.objects.filter(is_active=True)[:100]
    ...
    for product in products:
        # scrape_product(product.original_url)
        print(product.name)
